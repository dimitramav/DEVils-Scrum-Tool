...
<b-container class="Navigation" fluid>
    ...
    <b-row style="padding-right: 30px; padding-left: 30px">
        <b-col class="text-center">
            <b-form>
                ...
                <b-form-input class="input-style pcsprint" v-model="sprint.plan" type="text"
                            placeholder="Type the project's plan"></b-form-input>
                <b-row>
                    <p></p>
                </b-row>
                <label class="pcsprint">Goal:</label>
                <b-form-input class="input-style pcsprint" v-model="sprint.goal" type="text"
                            placeholder="Type the project's goal"></b-form-input>
                <b-row>
                    <p></p>
                </b-row>
                <label class="pcsprint">Deadline Date:</label>
                <div>
                    <b-form-input class="input-style pcsprint" id="inputLive" v-model.trim="sprint.deadlineDate"
                                type="date" :state="dateState"></b-form-input>
        ...
        <b-col>
            <b-row>
                ...
                <b-col>
                    <div>
                        ...
                        <b-form-select v-model="selected" :options="stories_options" class="mb-3 pcsprint"
                                    :select-size="4" style="min-width: 280px">
                        </b-form-select>
                    </div>
                </b-col>
            </b-row>
            ...
            <b-row>
                ...
                <b-col class="text-right">
                    ...
                    <b-form-select v-model="toremove" :options="selected_stories" class="mb-3 pcsprint"
                                :select-size="4" style="min-width: 280px">
                    </b-form-select>
                </b-col>
            </b-row>
    ...
    <b-row>
        <b-col class="text-center pcsprint">
            <b-button style="margin-top: 40px; padding-left:100px; padding-right: 100px;" type="submit" size="lg"
                        variant="success" @click="submit" :disabled="dateState===false">Create
            </b-button>
        </b-col>
    </b-row>
</b-container>
...
export default {
    ...
    methods: {
        submit() {
            ...
            let new_sprint = {
                deadlineDate: self.sprint.deadlineDate,
                Project_id: self.sprint.Project_id,
                goal: self.sprint.goal,
                plan: self.sprint.plan,
                isCurrent: self.sprint.isCurrent,
                numSprint: self.sprint.numSprint + 1
            };
            console.log("paparia");
            ...
            axios.post(self.$url + 'users/' + localStorage.getItem('userId') + '/projects/' + self.sprint.Project_id
                        + '/sprints', new_sprint, config)
                .then(function(response) {
                    ...
                    axios.patch(self.$url + 'users/' + localStorage.getItem('userId') + '/projects/' + self.sprint.Project_id
                                + '/pbis', self.pbis_list, config)
                        .then(function(response) {
                            // Debugging
                            console.log("xoxo")
                            self.$router.push({
                                path: '/project/' + self.sprint.Project_id + '/overview/'
                            })
                        })
        ...
        loadStories(idPBI, title) {
            ...
            const self = this;
            axios.get(self.$url + 'users/' + localStorage.getItem('userId') + '/projects/' + self.sprint.Project_id
                        + '/pbis?epicId=' + idPBI, {
                    headers: {
                        "auth": localStorage.getItem('auth_token')
                    }
        ...
        selectedStories(value) {
            ...
            var find_pbi = self.selected_stories.find(function(find_pbi) {
                return find_pbi.value === value;
            });
            if (find_pbi == undefined) {
                self.selected_stories.push({
                    value: value,
                    text: result[0].text
                });
            }
...
.form-frame {
    ...
    background: linear-gradient(-90deg, #d3d3d3, #d5d5d5);
