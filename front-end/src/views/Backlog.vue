    ...
    <b-container>
        <b-row>
            <b-col class="text-right">
                <div v-if="gotPBIS && currentPbis.length>0">
                    <b-btn v-b-modal="'new_storyundefined'" size="lg" variant="success"
                            class="m-md-2 pcsprint">Create New Epic</b-btn>
        ...
        <b-row>
            <b-col md="6" offset-md="3">
                <b-alert variant="success" align-h="center" :show="newEpicAlert"
                        dismissible>Epic created successfully!</b-alert>
            </b-col>
        </b-row>

        <b-row>
            <b-card-group v-for="cur_pbi in currentPbis" :key="cur_pbi.idPBI"
                ...
                <b-card :title="cur_pbi.title" img-top tag="article" class="mb-2">
                    <div slot="header">
                        <b-row>
                            ...
                            <!--New User Story-->
                            <div row>
                                <b-btn class="pcsprint" v-b-modal="'new_story'+cur_pbi.idPBI"
                                        style="height:98%">Add User Story</b-btn>
                                <new_pbi v-on:new_pbi="newUserStory" :idProject="currentProject_id"
                                            :Epic_id="cur_pbi.idPBI"></new_pbi>
                            </div>
                            <!--EDIT EPIC-->
                            <edit_pbi class="pcsprint" v-on:edit_pbi="editEpic" :idPBI="cur_pbi.idPBI"
                                        :idProject="currentProject_id" :title="cur_pbi.title" :desc="cur_pbi.description"
                                        :priority="cur_pbi.priority"></edit_pbi>
                        </b-row>
                    </div>
                    ...
                    <div row style="padding-top: 2px">
                        <b-btn class="pcsprint" v-b-toggle="'collapse'+cur_pbi.idPBI"
                                v-on:click="getEpicUserStories(cur_pbi.idPBI)" variant="info"> User Stories
                        </b-btn>
                    </div>
                    <b-collapse :id="'collapse'+cur_pbi.idPBI" class="mt-2 pcsprint">
                        <div>
                            ...
                            <b-jumbotron>
                                ...
                                <draggable v-model="currentUserStories[cur_pbi.idPBI]" :move="onUserStoryMove"
                                            @change="onUserStoryChange" :options="{group:'UserStories'}">
                                    ...
                                    <b-card-group v-for="cur_us in currentUserStories[cur_pbi.idPBI]" :key="cur_us.idPBI"
                                        ...
                                        <b-card v-if="cur_us.idPBI>0" class="mb-1">
                                            ...
                                            <b-collapse :id="'collapse'+cur_us.idPBI" visible accordion="my-accordion" role="tabpanel">
                                                ...
                                                <edit_pbi v-on:edit_pbi="editStory" :epicId="cur_pbi.idPBI"
                                                            :idPBI="cur_us.idPBI" :idProject="currentProject_id"
                                                            :title="cur_us.title" :desc="cur_us.description"
                                                            :priority="cur_us.priority"></edit_pbi>
...
import EditPBI from "../components/EditPBI.vue"
import NewPBI from '../components/NewPBI.vue'
...
export default {
    ...
    methods: {
        onUserStoryChange(input) {
            ...
            if (input.added) {
                ...
                if (this.currentUserStories[this.currentEpicId].length === 2) {
                    if (this.currentUserStories[this.currentEpicId][0].idPBI === -1) {
                        this.currentUserStories[this.currentEpicId].splice(0, 1);
                    }
                    else if (this.currentUserStories[this.currentEpicId][1].idPBI === -1) {
                        this.currentUserStories[this.currentEpicId].splice(1, 1);
                    }
                }
                ...
                axios.put(this.$url + 'users/' + localStorage.getItem('userId') + '/projects/' + this.currentProject_id
                            + '/pbis?isEpic=false', data, config)
        ...
        getPBIS() {
            const self = this;
            axios.get(this.$url + 'users/' + localStorage.getItem('userId') + '/projects/'
                        + this.$route.params.id + '/pbis?isEpic=true', {
                    headers: {
                        "auth": localStorage.getItem('auth_token')
                    }
                })
        ...
        getEpicUserStories(epicId) {
            ...
            axios.get(this.$url + 'users/' + localStorage.getItem('userId') + '/projects/' + this.$route.params.id
                        + '/pbis?isEpic=false&epicId=' + epicId, {
                    headers: {
                        "auth": localStorage.getItem('auth_token')
                    }
                })
