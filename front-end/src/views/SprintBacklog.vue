                                ...
                                <b-modal @ok="addTask(cur_story.idPBI)" class="text-left" :id="'new_task'+cur_story.idPBI" title="Add new task">
                                    <b-form>
                                        <b-form-group label="Description:" :label-for="'addTask'+cur_story.idPBI">
                                            <b-form-input :id="'addTask'+cur_story.idPBI" type="text" v-model="newTask_desc" required>
                                            </b-form-input>
                                        </b-form-group>
                                    </b-form>
                                </b-modal>
            ...
            <b-col class="bg-light">
                <draggable v-model="todoTasks" @change="addToTodo" :move=onMove :options="{group:'UserStories'}">
                    <transition-group name="markos1" class="list-group">
                        <b-card-group v-for="cur_task in todoTasks" :key="cur_task.idTask"
                            ...
                            <div v-if="cur_task.state===1 && cur_task.PBI_id===cur_story.idPBI">
                                <b-card img-top tag="article" class="mb-2">
                                    <div slot="header">
                                        <b-row align-h="start">
                                            <edit_task v-if="gotTasks" v-on:editTask="editTask" :desc="cur_task.description"
                                                    :PBI_id="cur_task.PBI_id" :state="cur_task.state" :Task_id="cur_task.idTask"></edit_task>
            ...
            <b-col>
                <draggable v-model="doingTasks" @change="addToDoing" :move=onMove :options="{group:'UserStories'}">
                    <transition-group name="markos" class="list-group">
                        <b-card-group v-for="cur_task in doingTasks" :key="cur_task.idTask"
                                deck style="margin-bottom: 10px; padding-left: 10px;" deck class="mb-2">
            ...
            <b-col class="bg-light">
                <draggable v-model="doneTasks" @change="addToDone" :move=onMove :options="{group:'UserStories'}">
                    <transition-group name="yolanda" class="list-group">
                        <b-card-group v-for="cur_task in doneTasks" :key="cur_task.idTask"
                                deck style="margin-bottom: 10px; padding-left: 10px;" deck class="mb-2">
...
export default {
    ...
    methods: {
        getSprintInfos() {
            const self = this;
            axios.get(this.$url + 'users/' + localStorage.getItem('userId') + '/projects/' + this.currentProject_id
            ...
            axios.put(this.$url + 'users/' + localStorage.getItem('userId') + '/projects/' + this.currentProject_id
                ...
                .then(function(response) {
                    ...
                    if (response.data.error) {
                        ...
                        //self.$emit('edit_pbi', self.idPBI, response.data.results.title, response.data.results.description,
                        //    response.data.results.priority, self.epicId);
