...
<b-container>
    <b-row>
        <b-col></b-col>
        <b-col class="w-100">
            <b-img style="width: 50%" src="https://cdn1.iconfinder.com/data/icons/flat-badges-vol-1/128/kanban-512.png"
                    v-on:click="gotoIndex"></b-img>
            <br><br>
            <b-form @submit="onSubmit" class="pcsprint">

                <b-form-group id="emailForm" label="Email address" label-for="email"
                            :invalid-feedback="validEmail===false ? 'Email is already in use' : ''"
                            description="We'll never share your email with anyone else.">
                    <b-form-input id="emailInput" type="email" v-model="form.email" @change="checkEmail"
                                :state="validEmail" autocomplete="off" required>
                        <!--autocomplete has a bug and v-model does not syncronize it-->
                    </b-form-input>
                </b-form-group>
                ...
                <b-form-group id="username" label="Username" label-for="username"
                    ...
                    <b-form-input id="username" type="text" v-model="form.username" @change="checkUsername"
                                :state="validUsername" autocomplete="off" required>
                        <!--autocomplete has a bug and v-model does not syncronize it-->
                    </b-form-input>
                </b-form-group>

                <b-form-group id="firstname" label="First Name" label-for="firstname">
                    <b-form-input id="firstname" type="text" v-model="form.firstname" required>
                    </b-form-input>
                </b-form-group>
                <b-form-group id="lastname" label="Last Name" label-for="lastname">
                    <b-form-input id="lastname" type="text" v-model="form.lastname" required>
                    </b-form-input>
                </b-form-group>

                <b-form-group id="password" label="Password" label-for="password"
                        description='Password must contain at least eight characters, at least one number and one special character'>
                    <b-form-input id="password" type="password" v-model="form.password" :state="checkPassword" required>
                    </b-form-input>
                </b-form-group>
                <b-form-group id="verify_password" label="Verify password" label-for="verify_password">
                    <b-form-input id="verify_password" type="password" v-model="form.verify_password" :state="verifyPassword" required>
                    </b-form-input>
                </b-form-group>
                <br>
                <b-button size="lg" type="submit" variant="primary"
                        :disabled="validEmail===false || validUsername===false || validPassword===false || verifiedPassword===false"> Sign up
                </b-button>
                <br><br><br>
...
export default {
    ...
    mounted() {
        ...
        if (localStorage.getItem('auth_token') !== 'null' && localStorage.getItem('userId') !== 'null') this.$router.push({
            path: '/'
        });
    },
}
