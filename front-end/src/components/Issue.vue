...
<div>
    <b-button @click="openModal()" class="pcsprint" variant="warning" style="background-color: rgba(242,192,0,0.83)">
        <b-img src="https://cdn2.iconfinder.com/data/icons/social-productivity-line-art-1/128/alert-triangle-512.png"
                style="width:20px; margin-right: 5px"></b-img>Issues
    </b-button>
...
export default {
    ...
    methods: {
        ...
        addIssue() {
            ...
            axios.post(this.$url + 'users/' + self.loggedUserID + '/projects/' + self.idProject + '/tasks/'
                        + self.idTask + '/issues', self.issueToAdd, {
                    headers: {
                        "auth": localStorage.getItem('auth_token'),
                        "Content-Type": 'application/json'
                    }
